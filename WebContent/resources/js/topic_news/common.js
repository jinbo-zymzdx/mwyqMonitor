...
function listUserWord(){
	var submenuhtml = "<li><a href='javascript:void(0);' onclick='addNode();'><span class='glyphicon glyphicon-plus'></span>  添加</a></li>";
...
//			alert(JSON.stringify(data));
	...
	for(var auwi = 0; auwi < alluserwords.length;auwi ++){
		...
		submenuhtml += "<div class='switch-div'><a title='"+word.word+"' value='"+word.id+"' onclick='wordSearch(\""+word.word+"\",\""+word_Type+"\",\""+word.lang_Type.trim()+"\");'>";
		...
		if(isPause == 1){
			if(bodyWidth < 1345){
				submenuhtml += '<div class="switch switch-mini monitor-switch" style="min-width:50px !important;"><input type="checkbox"></div>';
			}else{
				submenuhtml += '<div class="switch switch-mini monitor-switch"><input type="checkbox"></div>';
			}
		}else{
			if(bodyWidth < 1345){
...
function removeNode(object){
	 if(confirm("确定要删除该节点吗？")){
	 	var id = $(object).siblings("a").attr("value");
		$.ajax({
...
function saveMonitor(){
	...
	var html="";
	if(word != "" || word != null){
		html = "<li><div class='switch-div'><a title='"+describe+"' onclick='wordSearch(\""+word+"\",\""+word_type_code+"\",\""+lang_type_code+"\");'>"
...
//			showWord = word;
		...
		if(bodyWidth < 1345){
			html = "<li style='font-size:12px; padding:5px 5px 5px 10px !important;'><div class='switch-div'>"
					+"<a title='"+describe+"' onclick='wordSearch(\""+word+"\",\""+word_type_code+"\",\""+lang_type_code+"\");'>"

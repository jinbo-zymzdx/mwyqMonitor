...
;!function (win, $, undefined) {
	var SVGMap = (function () {
		...
		SVGMap.prototype.options = {
			...
			stateCountHtml: function (arrayList, key){
				for(var i=0; i<arrayList.length; i++){
					if(key == arrayList[i].name){
						return arrayList[i].count;
					}
				}
				return 0;
			},
		...
		SVGMap.prototype.render = function () {
			...
			//alert(JSON.stringify(opt.provinces,null,2))
			...
			for (var state in mapConfig.shapes) {
				...
				var r_factor = 0;
				var g_factor = 0;
				for(var pr=0; pr<opt.provinces.length; pr++){
					var name = opt.provinces[pr].name;
					
					if(state == name){
						var factor = Math.log(opt.provinces[pr].count).toFixed(2) * 10;
						
						var cred = 255 - factor;
						cred = cred < 100 ? 100 : cred;
						
						var cgree = 215 - (factor * 2);
						cgree = cgree < 0 ? 0 : cgree;
						
						var color = "rgb("+cred+","+cgree+",0)";
				...
				stateColor[state].hoverColor = hoverColor;		//鼠标移过的颜色
				...
				if (stateData[state] && stateData[state].diabled) {
					...
					obj.hover(function (e) {
						if (this != current) {
							this.animate({
								fill: stateColor[this.id].hoverColor	//鼠标移过时的颜色
							}, 250);
						}
						//鼠标移过省份显示省名
						if (opt.showTip) {
							...
							var count = opt.stateCountHtml(opt.provinces, this.provinceName);
							//alert(count)
							
							$('.stateTip').html(opt.stateTipHtml(stateData, this)+" : "+count);
							//$('.stateTip').html(opt.stateTipHtml(stateData, this)+" : 添加数据");
					...
					obj.mouseup(function (e) {		//鼠标点击
						...
						//this.animate({
							//fill: stateColor[this.id].selectedColor 	//点击后的颜色
						//}, 250);
